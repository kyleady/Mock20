...
var deck = createObj('deck', { name: 'Deck Name' }, { MOCK20override: true });
...
on('destroy:path', function (obj, prev) {
  log(obj.get('_type') + ' destroyed.');
});

on('change:path', function (obj) {
  log(obj.get('_type') + ' changed.');
});

on('change:path:rotation', function (obj, prev) {
  log(obj.get('_type') + ' rotation changed from '
   + prev.rotation + ' to ' + obj.get('rotation'));
});

on('change:path:rotation', function (obj, prev) {
  log(obj.get('_type') + ' rotation changed from '
   + prev.get('rotation') + ' to ' + obj.get('rotation') + ' (2nd function)');
});

on('add:path', function (obj) {
  log(obj.get('_type') + ' added.');
});
...
var player20 = createObj('player', { _displayname: 'player 20' }, { MOCK20override: true });
var tester = createObj('player', { _displayname: 'tester' }, { MOCK20override: true });
...
sendChat('The System', '/w Character Hello there, Character Name or character 20. '
 + 'It depends who was found first.');
tester.MOCK20chat('/w \"Character Name\" Hello there, Character Name.');
tester.MOCK20chat('/w everyman Hello there, everyone who controls everyman.');
sendChat('character|' + character.id,
'/w \"Character 20\" Hello there, player 20 through character 20.');
sendChat('The Tester', '/em shakes his fist.');
player20.set('speakingas', 'character|' + character20.id);
player20.MOCK20chat('[[d20]] [[[[d20]]d20]]');
player20.MOCK20chat('/r D20');
player20.MOCK20chat('/gr d20');
...
log('The recently deleted Path 7 disappeared only when we started rearranging objects. '
 + 'This is how Roll20 behaves and I sought to replicate it.');

log('==Player Is GM==');
player20.MOCK20gm = true;
...
log('==Journal==');
on('change:campaign:_journalfolder', function (obj) {
  log(obj.get('_journalfolder'));
});

log('Create Folder 1');
var folder1 = createObj('folder', { n: 'Folder 1' }, { MOCK20override: true });
log('Create Suspicious Note');
var handout = createObj('handout', { n: 'Suspicious Note' });
log('Create Folder 2');
var folder2 = createObj('folder', { n: 'Folder 2' }, { MOCK20override: true });
log('Move Folder 2 into Folder 1');
MOCK20moveToFolder(folder2, folder1.id);
log('Move Suspicious Note into Folder 1');
MOCK20moveToFolder(handout, folder1.id);
log('Move Folder 1 into the Root Folder');
MOCK20moveToFolder(folder1, 'root_folder');
log('Move Folder 1 before the Suspicious Note (inside of itself).');
MOCK20moveBeforeFolderItem(folder1, handout.id);
log('Move character 20 into Folder 2');
MOCK20moveToFolder(character20, folder2.id);
log('Move Folder 1 before Chracter Name');
MOCK20moveBeforeFolderItem(folder1, character.id);
log('Delete Folder 1');
folder1.remove({ MOCK20override: true });
log('Does the subfolder, Folder 2, exist anymore?');
log(getObj('folder', folder2.id, { MOCK20override: true }) ? 'Yep!' : 'Nope.');

log('==Jukebox==');
on('change:campaign:_jukeboxfolder', function (obj) {
  log(obj.get('_jukeboxfolder'));
});

log('Create Playlist 1');
var playlist1 = createObj('playlist', { n: 'Playlist 1' }, { MOCK20override: true });
log('Create Spooky Music');
var spooky = createObj('jukeboxtrack', { title: 'Spooky Music' }, { MOCK20override: true });
log('Create Happy Music');
var happy = createObj('jukeboxtrack', { title: 'Happy Music' }, { MOCK20override: true });
log('Create Sad Music');
var sad = createObj('jukeboxtrack', { title: 'Sad Music' }, { MOCK20override: true });
log('Create Playlist 2');
var playlist2 = createObj('playlist', { n: 'Playlist 2' }, { MOCK20override: true });
log('Move Spooky Music into Playlist 1');
MOCK20moveToPlaylist(spooky, playlist1.id);
log('Move Happy Music into Playlist 1');
MOCK20moveToPlaylist(happy, playlist1.id);
log('Sort Playlist 1');
MOCK20sortFolder(playlist1);
log('Move Sad Music into Playlist 2');
MOCK20moveToPlaylist(sad, playlist2.id);
log('Delete Playlist 1');
playlist1.remove({ MOCK20override: true });
log('Move Playlist 2 before Spooky Music');
MOCK20moveBeforePlaylistItem(playlist2, spooky.id);
